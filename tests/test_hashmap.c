...
static char* init_hashmap()
{
    ...
    mu_assert("capacity should be 0 (2)", HashMap.size(hm) == 0);
...
static char* add_elements()
{
    ...
    mu_assert("capacity should be 3 (1)", HashMap.size(hm) == 3);
    HashMap.put(hm, "Felix", 24);
    mu_assert("capacity should be 4 (2)", HashMap.size(hm) == 4);
    HashMap.remove(hm, "Felix");
    mu_assert("capacity should be 3 (3)", HashMap.size(hm) == 3);
    HashMap.free(hm);
    return 0;
}
...
static char* hash_collision()
{
    ...
    mu_assert("capacity should be 1 (1)", HashMap.size(hm) == 1);
    HashMap.put(hm, "Jacques", 3);
    mu_assert("capacity should be 1 (2)", HashMap.size(hm) == 1);
    HashMap.put(hm, "Jacques", 4);
    mu_assert("capacity should be 1 (3)", HashMap.size(hm) == 1);
...
static char* add_remove_and_hash_collision()
{
    ...
    mu_assert("capacity should be 2 (1)", HashMap.size(hm) == 2);
    mu_assert("value should be 1 (2)", HashMap.get(hm, jacques) == 1);
    HashMap.put(hm, jacques, 3);
    HashMap.put(hm, jacques, 3);
    mu_assert("capacity should be 2 (3)", HashMap.size(hm) == 2);
    mu_assert("value should be 3 (4)", HashMap.get(hm, jacques) == 3);
    HashMap.remove(hm, felix);
    HashMap.remove(hm, felix);
    mu_assert("capacity should be 1 (5)", HashMap.size(hm) == 1);
